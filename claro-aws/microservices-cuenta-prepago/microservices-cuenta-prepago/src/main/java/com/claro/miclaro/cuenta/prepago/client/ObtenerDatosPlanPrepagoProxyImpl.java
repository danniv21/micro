...
import com.claro.miclaro.cuenta.prepago.dto.ConsultaPlanPrepagoRequest;
...
@Repository
public class ObtenerDatosPlanPrepagoProxyImpl implements ObtenerDatosPlanPrepagoProxy{
	
	private static final Logger logger = LoggerFactory.getLogger(ObtenerDatosPlanPrepagoProxyImpl.class);

	@Override
	public ConsultaPlanPrepagoResponse obtenerdatosplan(String linea, ApplicationProperties properties,
			HttpHeaders httpHeaders) throws Exception {
		
		long tiempoInicio = System.currentTimeMillis();
		String msjTransaccion2 = "[obtenerdatosplan] ";
		
		ConsultaPlanPrepagoResponse response = null;
		String metodo = "obtenerdatosplan";
		String nombreWS = "consultaplanprepago";
		
		Gson gsonReq = new Gson();
		String uri = properties.getUriConsultaPlanPrepago();
		...
		try {
			...
			logger.info(msjTransaccion2 + "Tiempo de Timeout de Conexion: " + properties.getTimeOutConnectionConsultaPlanPrepago());
			logger.info(msjTransaccion2 + "Tiempo de Timeout de Ejecucion: " + properties.getTimeOutRequestConsultaPlanPrepago());
			
			ConsultaPlanPrepagoRequest request = new ConsultaPlanPrepagoRequest();
			MessageRequest messageRequest = new MessageRequest();
			
			HeaderRequest headerRequest = Utilitarios.getHeaderDataPower(httpHeaders, properties, properties.getHeaderModuloConsultaPlanPrepago(), properties.getHeaderOperationObtenerDatosPlan());
			...
			logger.info(msjTransaccion2 + "Tiempo total de proceso del llamado del servicio REST: {} Metodo: {} (ms):{} ", "/consultaplanprepago/obtenerdatosplan", metodo, System.currentTimeMillis() - tiempoInicioWS);
			
			response = gsonReq.fromJson(responseEntity.getBody().toString(),
					ConsultaPlanPrepagoResponse.class);
			...
			if (response.getMessageResponse() != null) {
				...
				if (!response.getMessageResponse().getBody().getResponseStatus().getCodigoRespuesta()
						.equals(Constants.CODIGO_CERO)) {
					throw new WSException(properties.getCodigoRespuestaIDF2(),
							properties.getMensajeRespuestaIDF2().replace("$me", metodo).replace("$ws", nombreWS));
				}
